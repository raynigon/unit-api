...
import com.raynigon.unit.api.jpa.exception.QuantityPackingException;
...
public class QuantityJavaDescriptor extends AbstractTypeDescriptor<Quantity<?>> {
    ...
    /**
     * Exposes the value of the shape attribute, used in
     * QuantityJavaDescriptor#getJdbcRecommendedSqlType
     *
    ...
    public Quantity<?> fromString(String string) {
        return UnitsApiService.reader().read(string);
    }
     ...
     * @throws QuantityPackingException thrown if the type is unknown to the implementation (String
     *                                  and Number types are supported)
     */
    @Override
     ...
     *
     * @param value   the value which should be used to create the new Quantity object, can either be a
     *                String or a Number
     * @param options unused
     * @throws QuantityPackingException thrown if the type of the given value is unknown to the
     *                                  implementation (String or Number are accepted types)
     */
    @Override
    @SuppressWarnings({"unchecked", "rawtypes"})
    public <X> Quantity<?> wrap(X value, WrapperOptions options) {
        if (value instanceof String && !StringUtils.isNumeric((String) value)) {
            return UnitsApiService.reader().read((String) value).to((Unit) unit);
