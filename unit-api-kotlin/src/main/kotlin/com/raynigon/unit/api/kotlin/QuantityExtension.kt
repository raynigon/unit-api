 ...
 */
fun <T : Quantity<T>> Quantity<T>.toValue(unit: IUnit<T>): Number = this.to(unit).value
 ...
 */
fun <T : Quantity<T>> Quantity<T>.toDouble(unit: IUnit<T>): Double = this.to(unit).value.toDouble()
...
fun <T : Quantity<T>> Quantity<T>.clamp(
    min: Quantity<T>,
    max: Quantity<T>,
): Quantity<T> = QuantityMath.clamp(min, this, max)
